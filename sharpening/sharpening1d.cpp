...
int* loadPixel(string targetFile, int i, int j){
    auto readin = chrono::steady_clock::now();
...
int main(int argc, const char * argv[]) {
    ...
    for(int a=0;a<i;a++){
        for(int b=0;b<j;b++){
            for(int rgb=0;rgb<3;rgb++){
                ...
                out[a*j*3+b*3+rgb] = filter(pixel, a+1, b+1, rgb, j);
            }
        }
    }
    ...
    auto readintime = chrono::duration_cast<chrono::microseconds>(filterloop - start);
